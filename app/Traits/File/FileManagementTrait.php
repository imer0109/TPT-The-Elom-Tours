...
trait FileManagementTrait
{
	...
	private function storeMultipleFiles(Request $request, string $fileKey, string $folderName): array
	{
		$files = $request->file($fileKey);
		$storedFiles = [];

		foreach ($files as $file) {
			$fileFullName = uniqid() . '.' . $file->getClientOriginalExtension();
			$storedFiles[] = Storage::disk(static::$disk)->putFileAs($folderName, $file, $fileFullName);
		}
