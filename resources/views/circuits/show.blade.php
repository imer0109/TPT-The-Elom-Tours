                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
                        ...
                        <div class="flex items-center mb-6">
                            ...
                            <span class="text-gray-600">{{ number_format($rating, 1) }}/5
                                ({{ $circuit->avis ? $circuit->avis->count() : 0 }} avis)</span>
                        </div>

                        <div class="flex flex-wrap gap-2">
                            @foreach ($circuit->categories as $categorie)
                                <span
                                    class="bg-gray-100 text-gray-800 px-3 py-1 rounded-full text-sm">{{ $categorie->nom }}</span>
                            @endforeach
                        </div>
                    </div>
                                ...
                                @foreach ($circuit->etapes as $etape)
                                    <div class="border-l-4 border-green-600 pl-4 py-2">
                                        <div class="flex items-center mb-2">
                                            <div
                                                class="bg-green-600 text-white rounded-full w-8 h-8 flex items-center justify-center mr-3">
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6">
                        ...
                        <div class="mb-8">
                            <div class="flex items-center mb-4">
                                <div class="text-yellow-500 text-xl mr-2">
                                    @php
                                        ...
                                        for ($i = 5; $i >= 1; $i--) {
                                            $starPercentages[$i] =
                                                $reviewCount > 0 ? round(($starCounts[$i] / $reviewCount) * 100) : 0;
                                        }
                                    @endphp
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-1 h-2 bg-gray-200 rounded-full mx-2">
                                        <div class="h-2 bg-yellow-500 rounded-full"
                                            style="width: {{ $starPercentages[5] }}%"></div>
                                    </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-1 h-2 bg-gray-200 rounded-full mx-2">
                                        <div class="h-2 bg-yellow-500 rounded-full"
                                            style="width: {{ $starPercentages[4] }}%"></div>
                                    </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-1 h-2 bg-gray-200 rounded-full mx-2">
                                        <div class="h-2 bg-yellow-500 rounded-full"
                                            style="width: {{ $starPercentages[3] }}%"></div>
                                    </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-1 h-2 bg-gray-200 rounded-full mx-2">
                                        <div class="h-2 bg-yellow-500 rounded-full"
                                            style="width: {{ $starPercentages[2] }}%"></div>
                                    </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="flex-1 h-2 bg-gray-200 rounded-full mx-2">
                                        <div class="h-2 bg-yellow-500 rounded-full"
                                            style="width: {{ $starPercentages[1] }}%"></div>
                                    </div>
                        ...
                        <div class="space-y-6">
                                    ...
                                    <div class="flex items-center mb-2">
                                        ...
                                        <div>
                                            <h4 class="font-semibold">
                                                {{ $review->user ? $review->user->name : $review->name }}</h4>
                                            <div class="flex items-center">
                                                ...
                                                <span
                                                    class="text-gray-600 text-sm">{{ $review->created_at->diffForHumans() }}</span>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            @empty
                                ...
                                <div class="border-b border-gray-200 pb-6">
                                    ...
                                    <p class="text-gray-700">"Une expérience inoubliable ! Notre guide était extrêmement
                                        compétent et passionné. Les villages visités étaient authentiques et l'accueil des
                                        habitants chaleureux. Je recommande vivement ce circuit pour découvrir la vraie
                                        culture togolaise."</p>
                                </div>
                            @endforelse
                        ...
                        <div class="mt-8 border-t pt-8">
                            ...
                            <form action="{{ route('circuits.review', $circuit->slug) }}" method="POST"
                                class="space-y-6">
                                @csrf
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label for="name" class="block text-gray-700 text-sm font-medium mb-2">Nom
                                            complet *</label>
                                    ...
                                    <div>
                                        ...
                                        <input type="email" id="email" name="email" value="{{ old('email') }}"
                                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 @error('email') border-red-500 @enderror"
                                            required>
                                        @error('email')
                                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                        @enderror
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center space-x-1">
                                        @for ($i = 1; $i <= 5; $i++)
                                            <label for="rating-{{ $i }}"
                                                class="cursor-pointer text-2xl text-gray-300 hover:text-yellow-500 relative">
                                                <input type="radio" id="rating-{{ $i }}" name="rating"
                                                    value="{{ $i }}" class="sr-only peer"
                                                    {{ old('rating') == $i ? 'checked' : '' }} required>
                                                <i
                                                    class="fas fa-star transition-colors duration-200
                    peer-checked:text-yellow-500
                    hover:text-yellow-400"></i>
                                            </label>
                                        @endfor
                                    </div>
                                    <span class="mt-1 text-sm text-gray-600" id="rating-value">
                                        @if (old('rating'))
                                            Vous avez noté : {{ old('rating') }} étoile{{ old('rating') > 1 ? 's' : '' }}
                                        @endif
                                    </span>
                                    @error('rating')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                ...
                                <div>
                                    <label for="comment" class="block text-gray-700 text-sm font-medium mb-2">Votre avis
                                        *</label>
                                    <textarea id="comment" name="comment" rows="4"
                                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 @error('comment') border-red-500 @enderror"
                                ...
                                <p class="text-sm text-gray-600">Votre avis sera publié après modération par notre équipe.
                                </p>
                            </form>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-lg shadow-md p-6 sticky top-6">
                        ...
                        <div class="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
                            ...
                            <span
                                class="text-green-600 font-bold text-xl">{{ number_format($circuit->prix, 0, ',', ' ') }}
                                FCFA</span>
                        </div>
                        <form action="{{ route('reservations.store', $circuit->slug) }}" method="POST"
                            class="space-y-4">
                            @csrf
                            <div>
                            ...
                            <div>
                                <label for="nombre_personnes" class="block text-sm font-medium text-gray-700 mb-1">Nombre
                                    de personnes</label>
                                <input type="number" id="nombre_personnes" name="nombre_personnes" min="1"
                                    value="1" required
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500">
                            </div>
                            ...
                            <div class="flex items-center justify-between pt-2 border-t border-gray-200">
                                ...
                                <span class="text-green-600 font-bold text-xl"
                                    id="prix-total">{{ number_format($circuit->prix, 0, ',', ' ') }} FCFA</span>
                            </div>
                        ...
                        <div class="mt-6 text-center text-sm text-gray-600">
                            ...
                            <a href="{{ route('contact.index') }}"
                                class="text-green-600 hover:text-green-800 font-medium">Contactez-nous</a>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition duration-300">
                            <div class="relative h-48 overflow-hidden">
                                @if ($relatedCircuit->images->isNotEmpty())
                                    <img src="{{ asset('storage/' . $relatedCircuit->images->first()->url) }}"
                                        alt="{{ $relatedCircuit->images->first()->alt }}"
                                        class="w-full h-full object-cover">
                                @else
                                    <img src="{{ asset('assets/images/placeholder.jpg') }}" alt="Image placeholder"
                                        class="w-full h-full object-cover">
                                @endif
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex justify-between items-center">
                                    <span
                                        class="text-green-600 font-bold">{{ number_format($relatedCircuit->prix, 0, ',', ' ') }}
                                        FCFA</span>
                                    <a href="{{ route('circuits.show', $relatedCircuit->slug) }}"
                                        class="text-green-600 hover:text-green-800 font-medium">Découvrir <i
    ...
    <script>
        ...
        stars.forEach(star => {
            star.addEventListener('change', function() {
                display.textContent = `Vous avez noté : ${this.value} étoile${this.value > 1 ? 's' : ''}`;
            });
        });
    </script>
