...
<div class="container px-6 mx-auto grid">
    ...
    <div class="p-4 bg-white rounded-lg shadow-md mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-red-600">{{ number_format($reservation->montant_restant + $paiement->montant, 2, ',', ' ') }} €</p>
            </div>
        </div>
    </div>
    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md">
        <form action="{{ route('admin.reservations.paiements.update', [$reservation, $paiement]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-6 mb-8 md:grid-cols-2">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="montant" name="montant" value="{{ old('montant', $paiement->montant) }}" min="0" max="{{ $reservation->montant_restant + $paiement->montant }}" step="0.01" class="block w-full pl-7 mt-1 text-sm border-gray-300 rounded-md focus:border-blue-400 focus:ring focus:ring-blue-200 focus:ring-opacity-50 @error('montant') border-red-500 @enderror" required>
                    </div>
                    @error('montant')
                ...
                <div>
                    ...
                    <select id="methode" name="methode" class="block w-full mt-1 text-sm border-gray-300 rounded-md focus:border-blue-400 focus:ring focus:ring-blue-200 focus:ring-opacity-50 @error('methode') border-red-500 @enderror" required>
                        ...
                        @foreach($methodes as $value => $label)
                            <option value="{{ $value }}" {{ old('methode', $paiement->methode) == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="datetime-local" id="date_paiement" name="date_paiement" value="{{ old('date_paiement', $paiement->date_paiement->format('Y-m-d\TH:i')) }}" class="block w-full mt-1 text-sm border-gray-300 rounded-md focus:border-blue-400 focus:ring focus:ring-blue-200 focus:ring-opacity-50 @error('date_paiement') border-red-500 @enderror" required>
                    @error('date_paiement')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea id="commentaire" name="commentaire" rows="3" class="block w-full mt-1 text-sm border-gray-300 rounded-md focus:border-blue-400 focus:ring focus:ring-blue-200 focus:ring-opacity-50 @error('commentaire') border-red-500 @enderror">{{ old('commentaire', $paiement->commentaire) }}</textarea>
                    @error('commentaire')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
